...
def monitor_performance(log, interval=1):
    """
    性能检测函数，定期记录系统和进程的性能信息。

    :param log: 日志记录器
    :param interval: 检测间隔（秒），默认为 1 秒
    """
    global is_over
    process = psutil.Process(os.getpid())  # 当前进程
    while True:
        # 获取系统性能信息
        ...
        # 记录性能信息到日志
        log.info(f"性能检测 - 系统 CPU 使用率: {cpu_percent}%")
        log.info(f"性能检测 - 系统内存使用率: {memory_percent}%")
        log.info(f"性能检测 - 系统磁盘读取: {disk_io.read_bytes} 字节, 磁盘写入: {disk_io.write_bytes} 字节")
        log.info(f"性能检测 - 进程 CPU 使用率: {process_cpu_percent}%")
        log.info(f"性能检测 - 进程内存使用: {process_memory_info.rss} 字节 (驻留集大小), {process_memory_info.vms} 字节 (虚拟内存大小)")
...
def start_process(args):
    ...
    if args.monitor_performance:
        log.info("启动性能检测...")
        performance_thread = threading.Thread(target=monitor_performance, args=(log, args.performance_interval))
        performance_thread.daemon = True  # 设置为守护线程，主程序退出时自动终止
    ...
    # 等待性能检测线程结束（如果启用）
    if args.monitor_performance and performance_thread is not None:
...
if __name__ == '__main__':
    ...
    parser.add_argument("--monitor_performance", type=bool, default=False, help="是否启用性能检测")
    parser.add_argument("--performance_interval", type=float, default=1.0, help="性能检测间隔（秒）")
