...
from expore_yml import SVDToYAMLConverter
...
def configure_unicorn(args):
    ...
    if args.default_yml == "false":  # gqz
        svd_data = SVDToYAMLConverter()
        yml_path_def = svd_data.convert(args.svd_path, "config.yml")
        config = load_config_deep(yml_path_def)
    else:
    ...
    for rname, region in sorted(config['memory_map'].items(), key=lambda r: 1 if r[1].get('overlay', False) else 0):
        ...
        for attr_name in ('base_addr', 'size'):
            if attr_name not in region:
                logger.error(
                    f"'{attr_name}' missing for region '{rname}'. Please complete the memory region definition.")
                sys.exit(1)
        ...
        if not region.get('overlay', False):
            if size & (globs.PAGE_SIZE - 1) != 0:
                logger.warning(
                    f"Size 0x{size:x} of region '{rname}' not page aligned. Aligning to next page boundary size.")
            ...
            for region_name, (other_start, other_size, _) in regions.items():
                ...
                if (
                    ...
                    logger.error(
                        f"Region '{rname}' (0x{start:x}-0x{own_end:x}) overlaps with region '{region_name}' (0x{other_start:x}-0x{other_end:x})")
                    sys.exit(1)
        ...
        if f:
            ...
            logger.info(
                f"Using file {str(f)}, offset {file_offset:08x}, load_offset: {load_offset:08x}, file_size: {file_size:08x}")
    ...
    mmio_ranges = [(start, start + size) for rname, (start, size, prot) in regions.items() if
                   rname.lower().startswith('mmio')]
    if not mmio_ranges:
    ...
    if args.dump_state_filename is not None:
        snapshot.init_state_snapshotting(uc, args.dump_state_filename, args.dump_mmio_states, mmio_ranges,
                                         args.dumped_mmio_contexts, args.dumped_mmio_name_prefix)
    ...
    if args.bb_trace_file is not None:
        trace_bbs.register_handler(uc, args.bb_trace_file, None,
                                   create_dynamic_filenames=args.dynamic_trace_file_revisions)
    ...
    # Native mmio fuzzing
    native.init(uc, mmio_ranges, exit_at_bbls, args.exit_at_hit_num, args.print_exit_info,
                args.fuzz_consumption_timeout, args.basic_block_limit)
...
def populate_parser(parser):
    parser.add_argument("default_yml", type=str, default="false", help="Whether to use the default configuration file")
    parser.add_argument("svp_path", type=str, default="", help="svd file path")

    parser.add_argument('input_file', type=str, help="Path to the file containing the mutated input to load")
    parser.add_argument('--prefix-input', dest='prefix_input_path', type=str,
                        help="(Optional) Path to the file containing a constant input to load")
    parser.add_argument('-c', '--config', default="config.yml",
                        help="The emulator configuration to use. Defaults to 'config.yml'")

    # Verbosity switches
    parser.add_argument('-d', '--debug', default=False, action="store_true",
                        help="Enables debug mode (required for -t and -M) (SLOW!)")
    parser.add_argument('-v', '--print-exit-info', default=False, action="store_true",
                        help="Print some information about the exit reason.")
    ...
    group.add_argument("-g", '--gdb', dest='gdb_port', type=int, default=0,
                       help='Spawn gdb server at specified port (experimental, known to have issues)')
    group.add_argument("-S", '--shell', default=True, action='store_false',
                       help='Invoke ipdb shell upon breakpoints (default)')

    # Argument-based limits
    parser.add_argument('-l', '--translation-block-limit', dest='basic_block_limit', type=int,
                        default=globs.DEFAULT_BASIC_BLOCK_LIMIT,
                        help=f"Maximum number of basic blocks to execute. 0: no limit. Default: {globs.DEFAULT_BASIC_BLOCK_LIMIT:d}")
    parser.add_argument('--fuzz-consumption-timeout', dest='fuzz_consumption_timeout', type=int,
                        default=globs.DEFAULT_FUZZ_CONSUMPTION_TIMEOUT,
                        help=f"Maximum number of basic blocks to execute while no input is read. 0: no limit. Default: {globs.DEFAULT_FUZZ_CONSUMPTION_TIMEOUT:d}")
    parser.add_argument('--interrupt-limit', dest='interrupt_limit', type=int, default=globs.DEFAULT_MAX_INTERRUPTS,
                        help=f"Maximum number of interrupts to raise. 0: no limit. Default: {globs.DEFAULT_MAX_INTERRUPTS:d}")
    parser.add_argument('--trace-event-limit', dest='trace_event_limit', default=globs.DEFAULT_TRACE_EVENT_LIMIT,
                        type=int,
                        help=f"Exit before the (n+1)th event id would be used. 0: no limit. Default: {globs.DEFAULT_TRACE_EVENT_LIMIT:d}")
    parser.add_argument('--exit-at', dest='exit_at_bbl', default=globs.EXIT_AT_NONE, type=sym_or_addr,
                        help="Exit at the given basic block address.")
    parser.add_argument('--exit-at-hit-num', dest='exit_at_hit_num', type=int, default=1,
                        help="Number of hits of basic block at which to exit. Defaults to 1 (exit on first hit).")
    ...
    parser.add_argument('--bb-set-out', dest='bb_set_file', default=None,
                        help="Trace (compact) set of visited basic blocks into binary file.")
    parser.add_argument('--mmio-set-out', dest='mmio_set_file', default=None,
                        help="Trace (compact) set of MMIO access contexts into binary file.")
    parser.add_argument('--bb-hash-out', dest='bb_hash_file', default=None,
                        help="Hash together all sequentially visited basic blocks into binary file. This can be used to compare executions.")
    parser.add_argument('--dynamic-trace-file-revisions', default=False, action="store_true",
                        help="Instead of overriding trace files for basic block traces, create numbered versions. Could be useful for debugging tracing issues.")

    # MMIO access state generation
    parser.add_argument('--state-out', dest='dump_state_filename', default=None,
                        help="Destination of output state(s). If all MMIO accesses are to be dumped, pass a directory here.")
    parser.add_argument('--dump-mmio-states', dest='dump_mmio_states', default=False, action='store_true',
                        help="Dump states at every unique MMIO access.")
    parser.add_argument('--dumped-mmio-contexts', default='',
                        help="Restrict the (pc, mmio_address) contexts for which to dump states for. Format: pc1:mmio1,pc2:mmio2,...,pcX:mmioX")
    parser.add_argument('--dumped-mmio-name-prefix', default='',
                        help="Add a prefix to each generated MMIO state name for distinguishability")
